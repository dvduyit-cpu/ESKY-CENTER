...
class LanguageCourseController extends Controller {
 ...
 public function update(Request $r,LanguageCourse $languageCenterCourse):RedirectResponse{$languageCenterCourse->update($this->data($r));$languageCenterCourse->classes()->update(['language_program_id'=>$languageCenterCourse->language_program_id,'language_level_id'=>$languageCenterCourse->language_level_id,'expected_sessions'=>$languageCenterCourse->sessions]);return redirect()->route('language-center-courses.index')->with('success','Đã cập nhật khóa học và đồng bộ chương trình, cấp độ, số buổi của các lớp liên quan. Học phí riêng của từng lớp được giữ nguyên.');}
 ...
 private function form(LanguageCourse $item):View{return view('language.center-courses.form',['item'=>$item,'programs'=>LanguageProgram::where(fn($query)=>$query->where('active',1)->orWhere('id', $item->language_program_id))->with('levels')->orderBy('name')->get()]);}
