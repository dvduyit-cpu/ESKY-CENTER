...
class LanguageClassController extends Controller
{
    ...
    private function form(LanguageClass $item):View{return view('language.classes.form',compact('item')+['courses'=>LanguageCourse::where(fn($query)=>$query->where('active',1)->orWhereKey($item->language_course_id))->with(['program','level'])->orderBy('name')->get(),'teachers'=>User::where(fn($query)=>$query->where('active',1)->orWhereKey($item->teacher_user_id))->orderBy('name')->get(),'students'=>LanguageStudent::whereIn('status',['new','waiting_class','studying'])->orderBy('name')->get()]);}
