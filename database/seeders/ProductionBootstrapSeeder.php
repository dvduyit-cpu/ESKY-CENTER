...
class ProductionBootstrapSeeder extends Seeder
{
    public function run(): void
    {
        ...
        DB::transaction(function () use ($email, $password, $name): void {
            ...
            $modules = collect([
                ...
                [$code,$name,$icon,$sortOrder] = $data;
                $module = Module::updateOrCreate(['code'=>$code], compact('name','icon')+['sort_order'=>$sortOrder]);
                return [$module->code=>$module];
            });

            foreach ($roles as $roleCode => $role) {
                foreach ($modules as $moduleCode => $module) {
                    $permission = $this->permissionFor($roleCode, $moduleCode);
            ...
            $personnel = Personnel::create([
                'code'=>'ADM001', 'name'=>$name, 'normalized_name'=>Str::ascii(Str::lower($name)),
                'type'=>'admin', 'position'=>'Admin hệ thống', 'email'=>$email,
                'default_kpi'=>0, 'has_kpi'=>false, 'is_consultant'=>false,
                'payment_type'=>'none', 'payment_value'=>0, 'active'=>true,
            ]);

            User::create([
                'personnel_id'=>$personnel->id, 'role_id'=>$roles['admin']->id,
                'name'=>$name, 'email'=>$email, 'password'=>Hash::make($password),
                'active'=>true, 'must_change_password'=>true, 'notifications_enabled'=>true,
            ]);
        });
    }
    ...
    private function permissionFor(string $role, string $module): array
    {
        ...
        if ($role === 'leader') {
            if ($module === 'software_settings') return $none;
            if ($module === 'work_tasks') return array_replace($all, ['can_export'=>false]);
