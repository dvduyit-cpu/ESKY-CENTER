...
<div class="row g-3 mb-4">
    ...
    <div class="col-sm-6 col-xl"><div class="card card-soft stat-card h-100"><div class="card-body p-4"><div class="d-flex justify-content-between gap-2"><div><div class="stat-label">{{$label}}</div><div class="fs-4 fw-bold text-{{$color}}">{{number_format($amount)}}đ</div></div><div class="stat-icon bg-{{$color}}-subtle text-{{$color}}"><i class="bi {{$icon}}"></i></div></div></div></div></div>
    @endforeach
</div>
...
<div class="row g-3 mb-4">
    <div class="col-sm-6 col-xl-3"><div class="card card-soft stat-card h-100"><div class="card-body p-4"><div class="d-flex justify-content-between"><div><div class="stat-label">Tổng khách hàng</div><div class="stat-value">{{number_format($totalLeads)}}</div></div><div class="stat-icon bg-primary-subtle text-primary"><i class="bi bi-person-lines-fill"></i></div></div><div class="metric-note">+{{$newLeads['month']}} khách trong tháng</div></div></div></div>
    <div class="col-sm-6 col-xl-3"><div class="card card-soft stat-card h-100"><div class="card-body p-4"><div class="d-flex justify-content-between"><div><div class="stat-label">Tổng học viên</div><div class="stat-value">{{number_format($totalStudents)}}</div></div><div class="stat-icon bg-success-subtle text-success"><i class="bi bi-mortarboard-fill"></i></div></div><div class="metric-note">+{{$registered['month']}} đăng ký trong tháng</div></div></div></div>
    <div class="col-sm-6 col-xl-3"><div class="card card-soft stat-card h-100"><div class="card-body p-4"><div class="d-flex justify-content-between"><div><div class="stat-label">Lớp đang hoạt động</div><div class="stat-value">{{number_format($activeClasses)}}</div></div><div class="stat-icon bg-warning-subtle text-warning"><i class="bi bi-easel2-fill"></i></div></div><div class="metric-note">{{$recruitingClasses}} lớp đang/sắp tuyển sinh</div></div></div></div>
    <div class="col-sm-6 col-xl-3"><div class="card card-soft stat-card h-100"><div class="card-body p-4"><div class="d-flex justify-content-between"><div><div class="stat-label">Tỷ lệ chuyển đổi</div><div class="stat-value">{{$conversionRate}}%</div></div><div class="stat-icon bg-danger-subtle text-danger"><i class="bi bi-graph-up-arrow"></i></div></div><div class="metric-note">{{$programCount}} chương trình hoạt động</div></div></div></div>
</div>
...
<div class="row g-3 mb-4">
    ...
    <div class="col-xl-4"><div class="card card-soft h-100"><div class="card-body p-4"><div class="d-flex align-items-center gap-3 mb-4"><div class="stat-icon bg-{{$color}}-subtle text-{{$color}}"><i class="bi {{$icon}}"></i></div><div><h5 class="mb-0 fw-bold">{{$title}}</h5><small class="text-muted">Theo thời gian hiện tại</small></div></div><div class="period-metrics"><div><span>Hôm nay</span><strong>{{$values['today']}}</strong></div><div><span>Tháng này</span><strong>{{$values['month']}}</strong></div><div><span>Năm {{$year}}</span><strong>{{$values['year']}}</strong></div></div></div></div></div>
    @endforeach
</div>
