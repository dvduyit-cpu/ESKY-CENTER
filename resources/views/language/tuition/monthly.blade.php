...
<form class="filter-panel row g-3 mb-4">
    ...
    <div class="col-lg-2 col-md-4"><label class="form-label">Trạng thái phiếu</label><select class="form-select" name="receipt_status"><option value="">Tất cả</option><option value="confirmed" @selected(request('receipt_status')==='confirmed')>Đã xác nhận</option><option value="pending" @selected(request('receipt_status')==='pending')>Chờ bổ sung</option></select></div>
...
<div class="alert alert-info border-0 shadow-sm"><i class="bi bi-info-circle-fill me-2"></i>Số liệu tháng được tính theo ngày nhận tiền. Học viên đóng nhiều lần ở nhiều tháng sẽ xuất hiện đúng tại từng tháng đã đóng.</div>
    ...
    <div class="table-responsive"><table class="table table-modern align-middle mb-0">
        ...
        @forelse($items as $payment)
            ...
            <tr>
                ...
                <td><span class="badge-soft {{$payment->receipt_status==='confirmed'?'badge-success':'badge-warning'}}">{{$payment->receipt_status==='confirmed'?'Đã xác nhận':'Chờ bổ sung'}}</span></td>
