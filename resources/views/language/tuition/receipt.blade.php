...
<main class="receipt">
    ...
    <table class="detail">
        ...
        <tbody>
            <tr><td><strong>Học phí {{$charge->course?->name}}</strong><span class="subtext">Mã khoản thu {{$charge->code}} · Lớp {{$classLabel}}</span></td><td class="money">{{number_format($charge->original_amount,0,',','.')}} đ</td><td class="money">{{\App\Support\ValueFormatter::percentage($charge->discount_percentage)}}%<span class="subtext">{{$charge->discount?->name?:'Không miễn giảm'}} · -{{number_format($charge->discount_amount,0,',','.')}} đ</span></td><td class="money">{{number_format($charge->payable_amount,0,',','.')}} đ</td><td class="money"><strong>{{number_format($payment->amount,0,',','.')}} đ</strong></td></tr>
