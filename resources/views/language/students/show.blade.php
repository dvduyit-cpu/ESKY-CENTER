...
$receiptStatuses=['confirmed'=>'Đã xác nhận','pending'=>'Chờ phiếu','cancelled'=>'Đã hủy'];
...
@if($classCharges->isNotEmpty())<h6 class="fw-bold mb-3">Lịch sử các lần đóng tiền</h6><div class="table-responsive mb-4"><table class="table table-modern"><thead><tr><th>Ngày đóng</th><th>Số phiếu</th><th>Số tiền</th><th>Hình thức</th><th>Người thu</th><th>Trạng thái phiếu</th></tr></thead><tbody>@forelse($classCharges->flatMap->payments->sortByDesc('paid_at') as $payment)<tr><td>{{$payment->paid_at?->format('d/m/Y H:i')}}</td><td>{{$payment->receipt_code?:'Chưa bổ sung'}}</td><td><strong>{{number_format((float)$payment->amount+(float)$payment->book_amount)}}đ</strong></td><td>{{$paymentMethods[$payment->payment_method]??$payment->payment_method}}</td><td>{{$payment->collector?->name?:'—'}}</td><td>{{$receiptStatuses[$payment->receipt_status]??$payment->receipt_status}}</td></tr>@empty<tr><td colspan="6" class="text-center text-muted">Chưa có lần đóng tiền.</td></tr>@endforelse</tbody></table></div>@endif
