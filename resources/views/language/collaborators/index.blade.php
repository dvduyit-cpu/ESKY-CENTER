...
<div class="card card-soft"><div class="table-responsive"><table class="table table-modern"><thead><tr><th>CTV</th><th>Liên hệ</th><th>Hoa hồng</th><th>Tổng số học viên giới thiệu</th><th>Trạng thái</th><th></th></tr></thead><tbody>@forelse($items as $i)<tr><td><strong>{{$i->name}}</strong><div class="small text-muted">{{$i->code}}</div></td><td>{{$i->phone?:'—'}}<div class="small text-muted">{{$i->email}}</div></td><td>{{$i->commission_rate}}%</td><td>{{$i->referred_students_count}}</td><td><span class="badge-soft {{$i->active?'badge-success':'badge-gray'}}">{{$i->active?'Hoạt động':'Ngừng'}}</span></td><td class="text-end text-nowrap"><a class="btn btn-sm btn-outline-secondary" href="{{route('language-collaborators.show',$i)}}" title="Xem học viên giới thiệu"><i class="bi bi-eye me-1"></i>Xem CTV</a><a class="btn btn-sm btn-outline-primary" href="{{route('language-collaborators.edit',$i)}}" title="Sửa CTV"><i class="bi bi-pencil"></i></a><form class="d-inline" method="POST" action="{{route('language-collaborators.destroy',$i)}}">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger" data-confirm="Xóa CTV này?"><i class="bi bi-trash"></i></button></form></td></tr>@empty<tr><td colspan="6"><div class="empty-state">Chưa có cộng tác viên.</div></td></tr>@endforelse</tbody></table></div>{{$items->links()}}</div>@endsection
@push('scripts')
<script>
