...
<body data-theme="{{ $systemTheme }}">
<main class="login-shell"><section class="login-panel" aria-labelledby="login-title">
<div class="login-intro">
<div class="login-brand"><span class="login-logo">@if($systemLogo)<img src="{{ asset($systemLogo) }}" alt="Logo">@else<i class="bi bi-mortarboard-fill"></i>@endif</span><span><strong>{{ $systemName }}</strong><small>Hệ thống quản lý trung tâm</small></span></div>
<div class="intro-content"><span class="intro-badge"><i class="bi bi-stars"></i> Quản lý tập trung, vận hành hiệu quả</span><h1>Mọi hoạt động của trung tâm trong một hệ thống.</h1><p>Quản lý tuyển sinh, học viên, lớp học và chỉ tiêu trên một nền tảng thống nhất.</p>
...
<div class="mobile-brand"><span class="login-logo">@if($systemLogo)<img src="{{ asset($systemLogo) }}" alt="Logo">@else<i class="bi bi-mortarboard-fill"></i>@endif</span><span><strong>{{ $systemName }}</strong><small>Hệ thống quản lý trung tâm</small></span></div>
