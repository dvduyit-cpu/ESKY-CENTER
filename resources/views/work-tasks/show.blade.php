...
@if($canPin)<div class="d-flex justify-content-end mb-3"><form method="POST" action="{{route('tasks.pin',$task)}}">@csrf @method('PATCH')<button class="btn {{$isPinned?'btn-warning':'btn-outline-secondary'}}"><i class="bi {{$isPinned?'bi-pin-angle-fill':'bi-pin-angle'}} me-1"></i>{{$isPinned?'Đã ghim công việc':'Ghim công việc'}}</button></form></div>@endif
        ...
        @elseif($canParticipate)
            <form method="POST" action="{{route('tasks.comments.store',$task)}}" enctype="multipart/form-data" @if($errors->any() && old('_task_form')==='comment-task') data-open-on-error @endif>
                ...
                <div class="task-file-box mb-2">
                    <label class="form-label mb-1"><i class="bi bi-paperclip me-1"></i>Tệp đính kèm</label>
                    <input class="form-control" type="file" name="attachments[]" multiple>
                    <small class="text-muted">Tối đa 5 tệp, mỗi tệp không quá 10 MB.</small>
                </div>
                ...
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-1">
                    ...
                    <div class="d-flex flex-wrap align-items-center justify-content-end gap-2">
                        ...
                        @if($comment->user_id===auth()->id() && $comment->created_at->gte(now()->subHours(24)))
                            <form method="POST" action="{{route('tasks.comments.retract',[$task,$comment])}}" data-confirm="Thu hồi phản hồi này? Nội dung và tệp đính kèm sẽ bị xóa.">
                                @csrf
                                @method('DELETE')
