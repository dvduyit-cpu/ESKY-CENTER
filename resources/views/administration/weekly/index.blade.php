...
<div class="administration-page" data-weekly-report data-review-url="{{ route('administration.weekly.check') }}">
    ...
    <div class="modal fade" id="createWeeklyPeriodModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-xl modal-dialog-scrollable modal-dialog-centered"><div class="modal-content"><form method="POST" action="{{ route('administration.weekly.periods.store') }}" data-assignee-form>@csrf
        ...
        <div class="modal-body"><div class="row g-3">
            ...
            <div class="col-md-8"><label class="form-label">Tên kỳ báo cáo</label><input class="form-control" name="title" maxlength="180" value="{{ old('title') }}" placeholder="Ví dụ: Tuần 32"></div>
            <div class="col-md-6"><label class="form-label">Mở nhận báo cáo từ</label><input class="form-control" type="datetime-local" name="starts_at" required value="{{ old('starts_at', now()->startOfWeek()->format('Y-m-d\T00:00')) }}">@error('starts_at')<div class="text-danger small">{{ $message }}</div>@enderror</div><div class="col-md-6"><label class="form-label">Khóa nhận báo cáo lúc</label><input class="form-control" type="datetime-local" name="ends_at" required value="{{ old('ends_at', now()->startOfWeek()->addDays(2)->endOfDay()->format('Y-m-d\T23:59')) }}">@error('ends_at')<div class="text-danger small">{{ $message }}</div>@enderror</div>
    ...
    @if(!$showEditor)
        @if(!$canManage && $selectedPeriod && !$submissionWindowOpen)
            <div class="weekly-window-closed"><i class="bi bi-lock-fill"></i><h4>Kỳ báo cáo đã khóa</h4><p>Kỳ báo cáo vẫn hiển thị để bạn xem nội dung đã lưu. Chỉ có thể nhập, chỉnh sửa hoặc gửi báo cáo trong thời gian đã quy định.</p></div>
        @endif
        <div class="weekly-report-list">
                ...
                @forelse($weekCards as $card)
                    ...
                    <article class="weekly-report-row {{ $submitted ? 'is-submitted' : 'is-pending' }}">
                        ...
                        <div class="weekly-row-status"><span class="activity-dot {{ $card->submission_window_open ? 'is-active' : '' }}" title="{{ $card->submission_window_status === 'open' ? 'Đang nhận báo cáo' : ($card->submission_window_status === 'upcoming' ? 'Chưa đến thời gian báo cáo' : 'Đã hết thời gian báo cáo') }}"></span><small>{{ $canManage ? $submitted.'/'.(int)$card->assigned_count.' đã gửi · '.(int)$card->draft_count.' nháp' : ($submitted ? 'Đã gửi' : ($personalReport ? 'Bản nháp' : 'Chưa báo cáo')) }} · <strong>{{ $card->submission_window_status === 'open' ? 'Đang nhận' : ($card->submission_window_status === 'upcoming' ? 'Chưa mở' : 'Đã khóa') }}</strong></small></div>
                        <div class="weekly-row-actions">
                            @if($canManage)
                                ...
                                @if($canSubmitReport && $card->assigned_to_current_user && $card->submission_window_open)<a href="{{ route('administration.weekly.index', ['period'=>$card->id,'open'=>1]) }}" title="{{ $personalReport ? 'Xem hoặc chỉnh sửa báo cáo của tôi' : 'Nhập báo cáo của tôi' }}"><i class="bi bi-pencil-square"></i></a>@elseif($canSubmitReport && $card->assigned_to_current_user)<button type="button" class="text-muted" disabled title="Đã ngoài thời gian báo cáo"><i class="bi bi-lock-fill"></i></button>@endif
                            ...
                            @else
                                @if($personalReport)
                                    <button type="button" data-bs-toggle="modal" data-bs-target="#weeklyReportModal{{ $card->id }}" title="Xem nhanh báo cáo"><i class="bi bi-eye"></i></button>
                                    @if($card->submission_window_open)<a href="{{ route('administration.weekly.index', ['period'=>$card->id,'open'=>1]) }}" title="Chỉnh sửa báo cáo"><i class="bi bi-pencil-square"></i></a><form method="POST" action="{{ route('administration.weekly.destroy', $personalReport) }}" onsubmit="return confirm('Xóa báo cáo tuần này? Nội dung đã nhập sẽ không thể khôi phục.')">@csrf @method('DELETE')<button type="submit" class="text-danger" title="Xóa báo cáo"><i class="bi bi-trash"></i></button></form>@else<button type="button" class="text-muted" disabled title="Đã ngoài thời gian báo cáo"><i class="bi bi-lock-fill"></i></button>@endif
                                @elseif($card->submission_window_open)<a class="weekly-enter-action" href="{{ route('administration.weekly.index', ['period'=>$card->id,'open'=>1]) }}" title="Nhập báo cáo"><i class="bi bi-pencil-square"></i><span>Nhập báo cáo</span></a>@else<button type="button" class="text-muted" disabled title="Đã ngoài thời gian báo cáo"><i class="bi bi-lock-fill"></i></button>@endif
                            @endif
                        </div>
                    </article>
                    ...
                    <div class="modal fade" id="weeklyReportModal{{ $card->id }}" tabindex="-1" aria-labelledby="weeklyReportModalLabel{{ $card->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered"><div class="modal-content weekly-preview-modal">
                            ...
                            <div class="modal-body">
                                @if($canManage)
                                    <div class="row g-3"><div class="col-sm-3"><div class="weekly-preview-stat"><span>Được giao</span><strong>{{ (int)$card->assigned_count }}</strong></div></div><div class="col-sm-3"><div class="weekly-preview-stat"><span>Đã gửi</span><strong>{{ (int)$card->submitted_count }}</strong></div></div><div class="col-sm-3"><div class="weekly-preview-stat"><span>Bản nháp</span><strong>{{ (int)$card->draft_count }}</strong></div></div><div class="col-sm-3"><div class="weekly-preview-stat"><span>Trạng thái</span><strong class="fs-6 {{ $card->submission_window_open ? 'text-success' : 'text-secondary' }}">{{ $card->submission_window_status === 'open' ? 'Đang nhận' : ($card->submission_window_status === 'upcoming' ? 'Chưa mở' : 'Đã khóa') }}</strong></div></div></div>
                                    <div class="small text-muted mt-3"><i class="bi bi-clock me-1"></i>Thời gian nhận báo cáo: {{ $card->submissionStartsAt()->format('H:i d/m/Y') }} – {{ $card->submissionEndsAt()->format('H:i d/m/Y') }}</div>
                                    <form id="periodEdit{{ $card->id }}" class="row g-3 mt-2" method="POST" action="{{ route('administration.weekly.periods.update', $card) }}">@csrf @method('PUT')<div class="col-12"><label class="form-label">Tên kỳ báo cáo</label><input class="form-control" name="title" maxlength="180" value="{{ $card->title }}"></div><div class="col-md-6"><label class="form-label">Mở nhận báo cáo từ</label><input class="form-control" type="datetime-local" name="starts_at" required value="{{ $card->submissionStartsAt()->format('Y-m-d\TH:i') }}"></div><div class="col-md-6"><label class="form-label">Khóa nhận báo cáo lúc</label><input class="form-control" type="datetime-local" name="ends_at" required value="{{ $card->submissionEndsAt()->format('Y-m-d\TH:i') }}"></div><div class="col-12"><div class="assignee-picker is-compact" data-assignee-group><div class="assignee-picker-head"><div><strong>Người phải báo cáo ({{ $card->assigned_count }})</strong></div><label class="form-check"><input class="form-check-input" type="checkbox" data-select-all-assignees><span class="form-check-label">Chọn tất cả</span></label></div><div class="assignee-picker-list">@foreach($reportableUsers as $person)<label class="assignee-option"><input class="form-check-input" type="checkbox" name="assigned_user_ids[]" value="{{ $person->id }}" @checked($card->assignedUsers->contains('id', $person->id))><span><strong>{{ $person->name }}</strong><small>{{ $person->email }}</small></span></label>@endforeach</div></div></div></form>
                            ...
                            <div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Đóng</button>@if($canManage)<button class="btn btn-outline-primary" type="submit" form="periodEdit{{ $card->id }}"><i class="bi bi-save me-1"></i>Lưu lịch</button>@endif<a class="btn btn-primary" href="{{ $canManage ? route('administration.weekly.summary', ['period'=>$card->id]) : route('administration.weekly.index', ['period'=>$card->id,'open'=>$card->submission_window_open ? 1 : null]) }}">{{ $canManage ? 'Mở trang tổng hợp' : ($card->submission_window_open ? 'Chỉnh sửa báo cáo' : 'Xem báo cáo') }}</a></div>
                        </div></div>
                    </div>
                    @endif
    ...
    @else
        <div class="d-flex justify-content-between align-items-center gap-3 mb-3"><a class="btn btn-outline-secondary" href="{{ route('administration.weekly.index') }}"><i class="bi bi-arrow-left me-1"></i>Về danh sách tuần</a><div class="small text-muted">Chỉ có thể nhập hoặc chỉnh sửa trong thời gian nhận báo cáo đã quy định.</div></div>
...
(() => {
    ...
    if (@json($errors->has('assigned_user_ids') || $errors->has('week_start') || $errors->has('starts_at') || $errors->has('ends_at'))) {
